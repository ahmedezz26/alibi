# Alibi — Progress & Current State

_Last updated: 2026-09-18. This is the single source of truth for where the project stands. Read it at session start; append dated entries at the end of §8. The *why* behind the project lives in `docs/PROJECT_BRIEF.md`._
...
- **The held-out set is now USED. Don't reuse it as a clean test.** Next steps (7+) are unblocked; see §7 and discuss with the user.
...
- **Judge = free `deepseek/deepseek-v4-flash-0731:free` with thinking OFF.** No paid calls without the user's explicit approval (see §6).

## 2. Hard rules / user decisions (do not violate)

1. **Money:** the user is budget-constrained and upset about spend. **Never run a paid model** without explicit approval for that specific run plus a cost estimate. The code enforces this: `make_judge` refuses non-`:free` models unless `ALIBI_ALLOW_PAID_MODELS=1`. Paid key balance: **$4.99 of $10** (it was $5.01 used, of which $2.21 was spent in the 2026-09-18 session).
2. **Frontier references come from papers, not runs.** The user explicitly said *not* to run GPT-5 ("take the results directly from the paper").
...
7. Don't commit or push without being asked. The repo is `git init`-ed but has **no commits yet**.
...
**No public labeled dataset has truly huge traces (>100K tokens after dedup).** Real huge traces will need the user's own production data (LangSmith project "Agentic" is empty) or a custom harness.
...
3. If the pipeline wins significantly, calibrate on the **train** split: the routing threshold, window size, CUSUM k/h, and the earliest-on-tie rule (it caused an early-step bias in v3). Then maybe a small frontier-model reference, **only with the user's approval and budget**.
...
- **2026-09-19, Jev stage 1 (chapter test, TRAIN, approved, spent ≈ $0.055):**
...
- **2026-09-19, stage 2 built** (free, TDD, 16 new tests; 58 pass). Not yet run on real traces. **Next: stage 3**, the full typed pipeline on the same 50 train traces (`runs/jev_stage1_ids.json`), ~$0.15, **waiting for approval.** Compare the chapter pick vs stage 1 (32/48).
- **2026-09-19, stage 3: typed pipeline on the same 50 TRAIN traces (approved, spent $0.123):** `runs/jev_stage3_train50.jsonl`. Paired with free DeepSeek runs on the same 50 (`runs/ds_train50_{whole,windowed}.jsonl`):
  ...
  - No thresholds were tuned (CUSUM defaults unchanged), so the test set stays clean. **Next: stage 4 on the 120 test traces, ~$0.30, waiting for approval.**
- **2026-09-19, stage 4: typed pipeline on the 120 long TEST traces** (the same traces as the long-120 DeepSeek runs; approved; spent $0.341 incl. reruns, **total Jev spend ≈ $0.53**). `runs/jev_stage4_test120.jsonl`.
...
- **2026-09-19, overfitting guard (the user asked "are we overfitting?"):**
  ...
  - **Pass bar, AGREED by the user on 2026-09-19 (fixed before any held-out run):** on the 126 held-out traces, the current Jev method vs a DeepSeek whole-trace call on the same traces: (a) more exact-step hits, paired sign test p<0.05; (b) ±3 hits not lower than whole-trace.
- **2026-09-19, the user decided:** improve the method first (on TRAIN only), then spend the held-out set once on the final version.
...
- **2026-09-19, rule choice on 100 TRAIN traces (v2 = readable non-ASCII + digit-aware counter + oversized-step clipping; spent ≈ $0.20):** `runs/jev_v2_train_{first50,fresh50}.jsonl`, 0 errors after fixes.
  ...
  - **Final version for held-out:** v2 code, `--combine-rule chapter_x_step`, window 10K. **Waiting for approval** of the held-out run (~$0.35 Jev + free DeepSeek whole baseline on the same 126) and the AgentRx cross-check (~$0.06).
- **2026-09-19, FINAL held-out run (approved; Jev $0.249 + AgentRx $0.061; total Jev spend ≈ $1.05).** Files: `runs/heldout126_{jev,ds_whole}.jsonl`, `runs/agentrx23_{jev,ds_whole}.jsonl`; the DeepSeek baselines were rerun with the v2 rendering, so both sides read the same text.
  ...
  - An OpenRouter paid-usage bump of +$0.007 seen that day was the user's own use elsewhere, not Alibi.
- **2026-09-19, AgentRx real-failure diagnosis (the user asked why real failures were missed; AgentRx is now a DEV set, not a clean test):**
...
- **2026-09-19, version choice run (approved ~$0.90; spent $0.70: dev80 v2 $0.228 + v3 $0.251; AgenTracer100 v3 $0.218). Total Jev spend ≈ $1.75.**
...
- **2026-09-19, the user chose to try ONE more idea before the final real-failure test. Pre-registered BEFORE building:**
...
- **Pass bar for the SWE-56 final real-failure test, AGREED by the user on 2026-09-19 (before running):**
...
- **2026-09-19, V4 "Jev-native" sensor check** (the user felt Jev was misused; TypeSafe guidance: narrow questions, only relevant context, JSON state with backtick paths, code composes). `runs/sensor_test.py` → `runs/sensor_v4_dev80.jsonl`.
  ...
  - **Spent $0.246 (approved ~$0.21; +17%, disclosed). Total Jev ≈ $2.59.** Calls average 0.74s (vs 1–3s for chunk calls).
  ...
  - **Confound:** the τ² policy/rules were clipped to 500 tokens to fit the budget, so relevant rules may have been cut.
...
- **2026-09-19, combining V2 + the V4 sensor (the user chose "combine"). Checked free on the stored dev80 answers BEFORE spending on the 350 fresh τ² traces (~$0.67 estimated):**
...
- **2026-09-19, cascade A→B (the user's idea: V2 shortlists its top-K agent steps, the V4 sensor picks among them). Free on stored dev80:** every K (2/3/5/8) × cue (severity/unsupported) is worse than V2. Best all80: 14 vs 21. τ² with K=8 and severity: the gold is in the shortlist 48/50 but B picks it only 5 times (random within the list ≈ 6).
...
- **2026-09-19, V7-code design run (paid, approved; $0.568 vs est. $0.76; 0 errors; 33 s/trace; 15 chunks/trace on average).** `runs/longrca_dev_code50_v7.jsonl`; scorer `runs/v7_report.py`. Router: 50/50 code.
...
- **2026-09-19, write-up rewritten (the user's brief):** V2 only, the ADAS framing (forward filter, CUSUM FDI, RTS-style smoother, gating/association, onset vs alarm), origin story, Jev as a sensor with designed channels (question = channel, answer type = measurement type), datasets and results, and comparison of accuracy/speed/cost/model. Doc: https://claude.ai/code/artifact/a06bb3b5-8df5-43ac-888d-c9937eda9cc4
- **2026-09-19, RCTA method vs V9: line-by-line diff (free; the user's request).** Source: arXiv 2608.15242 HTML, §5 + Appendix E (prompts and thresholds) + Appendix F (pseudocode), read verbatim (the alphaxiv MCP is not connected in this session). **No RCTA code is released**; the "run artifacts" with frozen prompts are not public, and only the leaderboard repo exists.
...
- **2026-09-19, V9-think (the user's go; free):** identical to V9 (same script, inputs, K, candidates and prompt), with ONE change: DeepSeek thinking on (`ALIBI_JUDGE_REASONING=high`, as in RCTA's final call). Output `runs/v9_think_{longrca,trajerrbench}.jsonl`. **Same pre-registered rule: PASS only if exact > 10 on LongRCA dev50 AND ≥ 21 on TEB dev80.** (TEB inputs are the same older-run rows as V9, to keep the test one-factor; the true V2 also scores 21 there.) Written before any result was seen.
  - **V9-think result (free; all 130 done; 4 empty free-tier responses retried; ~10 min per call with thinking on): FAILED.** LongRCA dev50 exact **8 vs V2 10** (same as V9 without thinking; ±3 12 vs 12); TEB dev80 **21 vs 21** (up from 19; ±3 37 vs 38). **Reasoning effort was not the missing ingredient on the long multi-agent coding traces.** The remaining untested differences from RCTA are its own candidate recall (up to 80 per trace), the subgoal outline, full-text candidates, the addressed-handoff rule for handoff candidates, and the repaired-error rule. V2 remains the method; RCTA chasing stops here unless the user asks.
- **2026-09-20, PRE-REGISTERED WebArena measurement (the user's go; Jev credit ≈ $0.21; no DeepSeek run, by the user's choice).** V2 (same settings as the SWE held-out) on `docs/heldout_longrca_webarena48_ids.json` = the first 48 of the 100 locked WebArena traces (median ~5 chapters). Estimate $0.166 (worst case +25% $0.21).
  ...
  - **Result (Jev $0.180 vs est. $0.166; 0 errors; 4.6 chapters/trace, median 38K tokens, 15.4 s/trace summed): V2 exact 6/48 = 12.5%** (95% CI 4.7–25.2%), ±3 10 (20.8%), ±5 12 (25.0%). Fisher vs published (177 traces): **trails RCTA** 33.3% (p=0.004); no clear difference vs ECHO 26.0% (p=0.054), All-at-once 16.9% (p=0.52), Step-by-step 11.3% (p=0.80), Binary search 5.1% (p=0.10); **beats FALAT** 2.3% (p=0.007). Point estimate below ECHO and all-at-once: on web tasks the method is **not better** than a whole-trace read on DeepSeek-V4-Flash. Jev credit left ≈ $0.03. The other 52 locked WebArena traces are unused.
