...
- **2026-09-20, the two deferred items closed.** `diagnose` now takes a `judge_factory` and owns the gate decision, so the gate no longer tokenizes the trace a second time. (First claimed as "1 pass, was 2", which was only true of gated traces: an analysed trace still rendered twice, once in the gate and once in `chunk_trace`. A gated trace now takes 1 render pass, down from 2; an analysed one takes 2, down from 3 - see the 0.1.4 entry for why sharing the render with `chunk_trace`, which would have made it 1, was reverted.) The "does this need a judge" test also exists in one place instead of being duplicated in both surfaces; `needs_judge` is gone with its last caller.
- **2026-09-20, 0.1.4: a fifth review; 15 findings, of which 13 fixed, 1 rejected on the evidence and 1 acknowledged and left alone.** The suite was green and ruff clean throughout, so none of these would have been caught by running it. Each was reproduced before it was fixed.
  - **Both "a gated trace never builds a judge" tests were inert.** They patched `make_judge` on the `cli` module, but `build_judge` looks it up on `localize`, so the patch intercepted nothing: the no-spend/no-send guarantee in SECURITY.md was certified by a test that could not fail. They now patch where the lookup happens, with the backend and spend guards satisfied so the gate is the only thing left that can stop a judge being built. Verified by breaking the gate: both fail, and passed again when it was restored.
  - **An injected judge sidestepped the spend guard.** `build_judge(settings, make)` checked only the backend, so the public `build_server(judge_factory=...)` could run paid Jev with `ALIBI_ALLOW_PAID_MODELS` unset - the docstring claimed the opposite. Backend and spend are now policy and run whoever builds the client; holding a key stays part of construction, so an injected judge brings its own.
  - **A judge that failed mid-analysis printed a traceback.** Narrowing the catch to `JudgeUnavailable` (fourth review) left nothing catching the 401 that `TypeSafeJudge` raises on the first chapter, because its client is built lazily. It now exits **3** - deliberately not the free refusal's 2 - and says the calls already made are billed, without blaming Jev for a failure that may be ours. Same message on the MCP surface.
  - **The shared render is reverted.** Passing `chunk_trace` the caller's render saved a pass, but it rendered and kept the whole trace *before* the gate ran, on the one path whose contract is that a refusal costs nothing, and it let the packing costs and the clip counter disagree whenever a caller passed its own `count_tokens` (measured: a chunk reporting 10 tokens while holding 437 unclipped chars). The gate is a streaming count again and `step_costs`/`prerendered` are gone; one extra render of an analysed trace is worth nothing against ~24 Jev calls.
  - **`diagnose`'s signature is 0.1.3's again.** `settings` had become required and keyword-only, so `diagnose(steps, judge, settings)` raised `TypeError` on a patch bump. It is positional once more and defaults to the environment.
  - **`--min-tokens`/`--max-tokens` stay, and so do abbreviations.** The **rejected** finding claimed only 0.1.2 shipped those spellings, which would have made the compatibility argument hollow; `git show v0.1.3:src/alibi/cli.py` shows both as aliases there too, so scripts on either release use them, and the test that pins them is named for 0.1.2 now. The **acknowledged** one follows from it: `--max-tokens` really does mean the window size on `score` and `eval` and the cost ceiling here, but removing it would break those scripts, so the collision is documented at the flag instead of fixed. What did go is `allow_abbrev=False`, added to hide the resulting ambiguity: it silently broke `--js`, `--sou` and `--proj` on `diagnose` alone, while every other subcommand kept them. `--min-t` now reports itself as ambiguous, which is what 0.1.3 did.
  - Smaller: the judge fake and the trace builder were duplicated between the localize and MCP tests (the copy in the MCP test recorded no calls, so cost and latency reaching the `Diagnosis` was untested, and its `kinds` list was sized 19 for a 9-step trace); both now come from `tests/fakes.py`. `FileNotFoundError` was redundant beside `OSError` in two except clauses.

- **2026-09-20, 0.1.4 continued: a sixth review; 5 findings, all fixed.** All five were in the fifth review's own fixes, which is the argument for reviewing again before the tag rather than after it.
  - **The new exit code 3 claimed money had been spent when none had.** The broad `except Exception` wrapped the whole of `diagnose`, so a settings error - `ALIBI_JUDGE_WINDOW_TOKENS=0`, which makes `chunk_trace` refuse - printed "the Jev calls already made are billed" and exited 3, with zero calls made. Reproduced on the command line, and on the MCP surface it would have been relayed to the user by a model as a spend event. The fix is an ordering one: the trace is chunked *before* the judge is built, so "a judge exists" now means "the trace is about to be sent". Everything that can fail for free fails at exit 2; the paid pipeline moved into `_analyse`, and a failure there becomes `AnalysisFailed`, carrying the number of calls that actually completed. The surfaces report that count rather than a blanket claim. Verified by restoring the old order: three tests fail.
  - **The failure detail was `repr(e)` from a third-party client.** That string can quote the request it was sent - i.e. the chapter of the user's trace, which SECURITY.md warns holds source code and secrets - and on the MCP surface it goes to a model. It is now the exception's type name plus its message, clipped to 200 characters.
  - **`tests/fakes.py` was untracked** while two test modules imported it at module level: committing without it would have broken collection of every gate and spend-guard test added in this pass. Staged.
  - **`test_settings_default_to_the_environment` was inert** - the same defect this pass exists to fix. It set `ALIBI_MIN_TRACE_TOKENS=1000000` and asserted `gated`, but the fixture trace is ~1K tokens and so is gated under the 50K default anyway; it passed whether or not the environment was read. It now sets a ceiling of 1 and asserts the refusal names it, which is unreachable unless `load_settings()` ran.
- **2026-09-20, 0.1.4 continued: a seventh review; 5 findings, all fixed, none blocking.** Again all five were in the previous round's fixes; the reviewer confirmed the two big ones from round six (the ordering fix and the spend guard) hold.
  - **The cost numbers were the judge's lifetime totals, not this trace's.** `judge_calls`, `cost_usd`, `judge_seconds` and `AnalysisFailed.completed` all summed the whole of `judge.calls`. Nothing requires a host's `judge_factory` to return a *fresh* judge - reusing one client is the natural thing to do - so the fifth `diagnose_trace` call would have reported the cost of all five traces, and a failure would have claimed "112 Jev call(s) completed and are billed" for a run that made 24. These numbers are a money claim about one trace, so they are counted from the judge's state on entry now. Verified by removing the offset: the reuse test fails.
  - **The MCP surface relayed the judge client's own error message.** Round six clipped it and dropped the repr, but `str()` of a pydantic `ValidationError` renders `input_value='<chapter text>'`, so trace content - which SECURITY.md says holds source and secrets - could reach the model that called the tool. The client now gets the exception *type* and the cost; the cause goes to the server's stderr, which stays on the machine. The CLI still prints it in full, since that is the user's own terminal and their own trace. Verified with a judge that raises the chapter back: the rendered `xxxx…` content is in the stderr line and absent from the tool error.
  - **`PAID_HELP` named only `ALIBI_ALLOW_PAID_MODELS`,** and since round five's guard runs before `make_judge`, a user who set the backend and followed the message exactly would have hit `TYPESAFE_API_KEY is not set` as a second failure - the same two-step dead end 0.1.3 fixed for `BACKEND_HELP`. It names both now, and says why it cannot tell them whether the key is there.
  - **The free settings error quoted internal parameter names.** `ALIBI_JUDGE_WINDOW_TOKENS=0` produced "overlap_tokens must be smaller than max_tokens", neither of which the user can find in the docs. It is now a `SettingsError` naming the variable they set. Its own type also lets both surfaces stop catching bare `ValueError`, which would have quietly presented any internal one as a user configuration error.
  - **"Part of the trace has left the machine" was an overclaim.** The judge can fail client-side before any request - `to_typesafe` rejects a question type, for instance - so at 0 completed calls the message now asserts neither the spend nor the send, in the code, the README and the exception's own docstring.
- **2026-09-20, 0.1.4 continued: an eighth review; 4 findings, all fixed, two of them reproduced by the reviewer.** The ordering fix, `SettingsError`, exit 3 and the per-trace offset were all re-verified as sound.
  - **A judge that keeps no call log reported "nothing is billed" after billing.** `Judge` promises only `evaluate`; `calls` is a convention the two shipped backends follow, and `build_server(judge_factory=...)` exists so a host can wire up its own client. Reproduced: 10 real calls, then a failure, and the user was told "No Jev call completed, so nothing is billed", with `judge_calls=0, cost_usd=0.0` on the success path. The count is now `None` for such a judge, the message says the cost is unknown, and `Diagnosis`'s three cost fields are nullable - unknown is not zero when the number is a money claim.
  - **The "a bad window costs nothing" guard only caught `window <= 0`.** `chunk_trace` raises only when the overlap (`window // 8`) is not smaller than the window, which is true for 0 alone. A user who writes `ALIBI_JUDGE_WINDOW_TOKENS=10` meaning 10,000 got one chapter per step: measured on a 40-step trace, windows of 1/5/7/8/10 all produced 40 chapters and ~80 billed calls, with the gate's estimate (2 reads per token) saying it would be cheap. Now refused for free when no step shares a chapter with another **and** the window is under a tenth of the measured 10,000 - the second condition on purpose, since one step per chapter is legitimate when the steps themselves are bigger than a chapter, which has its own test.
  - **The blanket `except Exception` made Alibi's own bugs look like a Jev outage.** A `KeyError` in `backward_pass_typed` and a dropped connection were reported identically, with no stack on either surface, so a real bug was unreportable. Both surfaces now print the traceback to stderr - the MCP server's stderr stays on the machine - and keep the cost as the last line, where a user looks.
  - **The per-trace offset is right for sequential reuse and wrong for concurrent reuse.** Two diagnoses sharing one judge interleave in one call log, and each would report the other's spend. Documented as the contract on `diagnose` and `build_server` (one judge per diagnosis is always safe) rather than tagging every `CallRecord`, since neither shipped surface shares a judge: the CLI and the MCP tool each build one per call.
- **2026-09-20, the product is now pinned to the measured configuration** (`tests/test_product_matches_research.py`). Every number above came from `alibi eval` (`evaluate_windowed`); users get `alibi.localize.diagnose`. They share `forward.py`, `backward.py` and `drift.py` but each has its own wrapper choosing the chapter size, card, checks, combine rule, pick and CUSUM thresholds, and nothing compared the two - editing `V2_CONFIG` left the whole suite green while the product silently became a variant that was never measured, still carrying the measured claim. The test runs one trace through both wrappers with the same deterministic judge and asserts the same predicted step, chapter count, anchor, alarm and ranking, plus that both asked the judge exactly the same questions (which is what catches a change to the `card`, since no answer depends on it). The research settings are written out as literals, not imported from the product, or it would agree with whatever the product does. Verified by mutating each knob in turn: `card`, `checks`, `rule`, `pick`, `cusum_k` and `cusum_h` all fail it, as does hardcoding `checks=False` at the call site, which the configuration assertion alone cannot see.
- **2026-09-20, README verified line by line against the code, and four things it got wrong.** Every documented command was executed: the gated example (219 tokens, word for word), the Claude Code session one-liner (resolved this session's own 6.8MB transcript to 333,562 tokens), the directory sweep, the LangSmith form, `uvx --from 'agent-alibi>=0.1,<0.2' alibi diagnose` against the published wheel, and the plugin's MCP server driven over a real stdio handshake (initialize -> tools/list -> tools/call, returning the Diagnosis). Fixed: (a) the TrajErrBench median was written as 59K; recomputed from `runs/heldout_swe56_v2.jsonl` it is **57,354**, so 57K - the worked example in the README checks out exactly against that file (118 steps, 10 chapters, alarm 6, gold = pred = 74, 80,778 tokens). (b) The chapter size was stated as a constant "10K-token chapters" in the diagram, the stage table and the output guide; it is `ALIBI_JUDGE_WINDOW_TOKENS`, whose default of 10,000 is what every measurement used, and it now reads that way. (c) Cost was rounded up to "$0.10 per million trace tokens"; the estimate is 2 x $0.042 = **$0.084**. (d) `--source langsmith` raised the SDK's own `LangSmithNotFoundError` as a traceback with exit 1 for the ordinary mistakes (missing key, mistyped project, unknown id) - the file adapters were wrapped in `TraceFormatError` but the LangSmith branch returned before the wrapper. Now wrapped, exit 2, with a test. Also documented: a working Claude Code session is usually over the 250K ceiling and is refused with its cost, which the README now shows instead of leaving the reader to discover; and `.env` fills any variable absent from the shell, which is how a run can spend when you thought you had unset the key.
- **2026-09-20, 0.1.4 released.** What it changes for a user of 0.1.3, in one place. **Money:** a failure part-way through an analysis no longer claims a spend that did not happen - the trace is chunked before the judge is built, so a settings error costs nothing and exits 2, while a real mid-analysis failure exits 3 and names the calls that completed. A judge that keeps no call log reports null rather than $0.00. A window far too small to group steps (the `ALIBI_JUDGE_WINDOW_TOKENS=10` typo, meaning 10,000) is refused for free instead of billing one call per step. **Privacy:** the MCP surface no longer relays the judge client's own error text, which can quote the chapter it was given; the client gets the exception type and the cost, the cause goes to the server's stderr. **Errors:** `--source langsmith` reports a missing key, mistyped project or unknown id as a clean exit 2 instead of a traceback from inside their SDK, and the paid-model refusal names `TYPESAFE_API_KEY` as well, so following it cannot hit a second failure. **API:** `diagnose(steps, judge, settings)` is positional again, as in 0.1.3. **Docs:** every command in the README was executed before it shipped, against the published package and the plugin's MCP server; the TrajErrBench median is 57K (recomputed), the chapter size is documented as a setting whose default is 10,000 rather than a constant, and install and usage are numbered steps with their real output. **Tests:** 183, including `tests/test_product_matches_research.py`, which fails if the shipped configuration stops matching the one the benchmarks measured.
